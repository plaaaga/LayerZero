...
class WindowName:
    ...
    def update_name(self):
        if os.name == 'nt':
            windll.kernel32.SetConsoleTitleW(f'LayerZero2 [{self.accs_done}/{self.accs_amount}] | {self.path}')
...
def cround(number: float, digits: int):
    if int(number) == number: return int(number)
...
def choose_mode():
    from_cex = "/".join(settings.WITHDRAW_PARAMS["exchange"])
    bridge_times = '-'.join([str(v) for v in settings.BRIDGE_PARAMS["bridges_amount"]])
    questions = [
        List('prefered_path', message="Choose action",
             choices=[
                '(Re)Create Database',
                f'1. {from_cex} -> {bridge_times} Bridges -> Exchange',
